...
static void module_load(void)
{
   ...
   // signal(SIGALRM, alarm_handler);
    ...
    newact.sa_handler = alarm_handler;//给newact设置新的行为
    sigemptyset(&newact.sa_mask);//把newact的信号集清空
    sigaddset(&newact.sa_mask, SIGALRM);//将SIGINT信号添加到newact的信号集中
   // sigaddset(&newact.sa_mask, SIGRTMIN);//将SIGRTMIN信号添加到newact的信号集中
    newact.sa_flags = 0;//newact无特殊需求

    sigaction(SIGALRM, &newact, NULL);//给SIGINT信号设置新的行为
   // sigaction(SIGRTMIN, &newact, NULL);//给SIGRTMIN信号设置新的行为
